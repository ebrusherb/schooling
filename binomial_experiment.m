...
box off
xlabel('Proportion of informed neighbors','FontName',fontname,'FontSize',textfontsz)
...
%     part_diag6=1.5*T*eye(N)+T^2/2*Lambda;
    ...
    part_diag6(Lambda<=-1.9)=1/4*(-1/2*(exp(-2*T)-1));
...
clc
[corr(particular,props,'type','spearman') corr(particular,real(part5),'type','spearman')  corr(particular,real(part6),'type','spearman') corr(particular,real(part7),'type','spearman')]
