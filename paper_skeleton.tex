...
\vspace{10pt}
%\\ Eleanor Brush, Naomi Leonard, Simon Levin
\end{center}
...
\end{align*}
%Therefore, the probability of having an opinion $x$ is 
%\begin{equation*}
%P\big(v_i(1) =x\big)=P\big(\text{$i$ has $(2x-1)k_i$  informed neighbors}\big).
%\end{equation*}
%for $x=\frac{1}{2},\frac{1}{2}\frac{k_i+1}{k_1},\dots,\frac{1}{2}\frac{k_i+k_i}{k_i}=1$.
...
%Note that since $R^TR=RR^T=I_N$, $Q^TQ=I_N-\frac{1}{N}\vec{1}\vec{1}^T$ and $QQ^T=I_{N-1}$. 
...
%Since $y=Qz$, if the same distribution of the opinion vector is used to find both $C_z$ and $E[||y||]$, then $E[||y||]=E[y^Ty]=\Tr(E[y^Ty])=\Tr(E[yy^T])=Tr(QC_zQ^T)$, so the two group properties are closely related. 
...
%\begin{equation*}
$H(v)=-v^TPv \text{ without the signal and } H_\text{s}(v)=-(v-\vec{1})^TP_\text{s}(v-\vec{1}) \text{ with the signal. }$%\label{Hamiltonian}
%\end{equation*}
...
\begin{equation*}
\rho=\frac{1}{\lim_{t\to\infty}\sqrt{E[y(t)^Ty(t)]}}=
%\frac{1}{\lim_{t\to\infty}\sqrt{E[\Tr(y(t)^Ty(t))]}}=
\frac{1}{\lim_{t\to\infty}\sqrt{E[\Tr(y(t)y(t)^T)]}}=
\frac{1}{\lim_{t\to\infty}\sqrt{E[\Tr(\Sigma_y)]}}.
\end{equation*}

%%%%%%%%%%
%The first measurement of the flock's ability to reach consensus is what has been called the $\mathscr{H}_2$ robustness of the network, which measures how far away from consensus the flock is at equilibrium.  Specifically, we consider the noisy opinion dynamics of the form 
%\begin{equation*}
%\dot{v}_i=\sum_jL_{ij}v_j+\sqrt{k_i}\xi_i,
%\end{equation*}
%where $\xi_i$ is a random variable such that the vector $\xi(t)$ satisfies $E[\xi(t)]=\vec{0}$ and $E[\xi(t)\xi(s)^T]=\frac{1}{2}\delta(t-s)I$ where $I$ is the $N\times N$ identity matrix.  In the absence of noise, the system would reach consensus with each $v_i=\alpha$ for some $\alpha\in\R$.  To consider deviations from this consensus state, it is useful to define a matrix $Q\in\R^{N-1}\times\R^N$ such that each row of $Q$ is orthogonal to $\vec{1}$ and the rows are pairwise orthogonal. Young et al. show in \cite{Young:2010fk,Young:2013kx} that this is equivalent to a $Q$ such that $QQ^T=I_{N-1}$ and $Q^TQ=I_N-\frac{1}{N}\vec{1}\vec{1}^T$. 
%
%There are two ways to describe equilibrium properties of a flock sharing information over a network.  The first is to write down a set of differential equations with noise to describe how each bird's opinion changes over time and then to analyze the distribution of these opinions after the dynamics have reached equilibrium \cite{Young:2010fk,Young:2013kx}. The second is to write down an explicitly probability distribution of the opinions at equilibrium and to analyze this distribution. With either approach, we consider a vector of opinions $v$ where $v_i$ a a scalar that represents the $i^\text{th}$ bird's opinion about an unspecified behavior, for example where or how fast to fly. Both approaches rely on a network of connections between the birds, which we will describe first. Let $A$ be the adjacency matrix such that 
%
%\begin{equation}
%A_{ij}=\left\{\begin{array}{l l l}
...
%because this is the convergence stable strategy 
...
%
%Surprisingly, even without imposing costs on paying attention to more neighbors, paying attention to the whole flock is not always the best strategy.  Surprisingly, when the signal becomes more public, more social information is better.
%
...
%it is by no means guaranteed that selection acting on individuals' learning abilities will lead them to use strategies that are optimal from the group's perspective, but it is possible.

%caveats: scalar opinion, agnostic to what that opinion refers to in reality, static network as flock responds to environmental signal 

%expect our results to hold for metric neighborhoods as well as topological networks
\newpage
\section{Appendix}
