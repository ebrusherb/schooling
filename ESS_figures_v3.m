...
maxrec=zeros(1,2);
invader_scores=cell(N,2);
invader_counts=zeros(N,2);
invader_minscores=cell(N,2);
resident_scores=cell(N,2);
resident_counts=zeros(N,2);
resident_minscores=cell(N,2);
ninv=zeros(3,2);
nres=zeros(3,2);
probeaten=zeros(N,2);
...
totalprob_invader=zeros(N,2);
totalprob_resident=zeros(N,2);
...
for j=1:2
    ...
    p=zeros(1,2);
    if j==1
        p(1)=plot(1:N,totalprob_resident(:,j),'-o','Color',cols(1:3,1),'LineWidth',3);
        p(2)=plot(1:N,totalprob_invader(:,j),'-o','Color',cols(1:3,2),'LineWidth',3);
    else
        p(1)=plot(1:N,totalprob_resident(:,j),'-o','Color',cols(1:3,2),'LineWidth',3);
        p(2)=plot(1:N,totalprob_invader(:,j),'-o','Color',cols(1:3,1),'LineWidth',3);
    end
    set(gca,'xlim',[1 max(maxrec)],'ylim',[0 max(max(totalprob_invader,[],1))])
