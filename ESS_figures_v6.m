...
legend('boxoff')
v=get(leg,'Position');
set(leg,'Position',[.18 .73 v(3) v(4)])
...
v=get(gca,'Position')
v=get(c,'Position')
set(c,'Position',[.92 .15 .02 .3],'Fontname',fontname,'FontSize',labfontsz)
% ylim=get(c,'YLabel');
...
for i=1:n
    ...
    plot(corrlengths(end,i)*ones(l,1),ESSseaten{t,i},'o')
end
...
fitnesseaten=fitnesseaten((end+1-Lh):end,(end+1-14):end);
tfitness=transpose(fitnesseaten);
offset=1;
below=Lh*offset+1:(Lh+1):Lh^2;
above=offset+1:(Lh+1):(Lh^2-Lh*offset);

figure
hold on
plot(corrlengths(1:(end-offset),r),tfitness(above),'-o')
plot(corrlengths(offset+1:end,r),tfitness(below),'-o')
plot(get(gca,'xlim'),ones(2,1))