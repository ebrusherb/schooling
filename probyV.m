function p = probyV(y,V,Pf,Pftilde)
N=length(y);
eN=[zeros(N-1,1);1];
...
p=sqrt(N)/sqrt((2*pi)^N*det(inv(-Pf)))*exp(-1/2*(transpose(y)*(Pftilde-1/Pftilde(N,N)*Pftilde(:,end)*Pftilde(end,:))*y))*exp(-1/2*Pftilde(N,N)*N*(V-1+Pftilde(end,:)*y/Pftilde(N,N)/sqrt(N))^2);
end